...
def build_header(feature, qualifier_list):
    ...
    header.append(organism.replace(' ', '_'))
    #header.append(subclass.replace(' ', '_'))
    #header.append(product_1.replace(' ', '_'))
...
#file_name = os.path.splitext(os.path.basename(out_file_handle.name))[0]
record_iterator = SeqIO.parse(in_file_handle, file_format)

for record in record_iterator:
    print "Converting '%s' to fasta ..." % record.description
 #   print ''.join(list(chain.from_iterable([feature.qualifiers['strain'] for feature in record.features if feature.type == 'source' ])))
    org = str(''.join(list(chain.from_iterable([feature.qualifiers['organism'] for feature in record.features if feature.type == 'source' ]))))
    try:
        strain = str(''.join(list(chain.from_iterable([feature.qualifiers['strain'] for feature in record.features if feature.type == 'source' ]))))
    except:
        strain = org
    try:
        subclass = str(''.join(list(chain.from_iterable([feature.qualifiers['label'] for feature in record.features if feature.type == 'subregion' ]))))
    except:
        subclass = " "
    try:
        product_1 = str(''.join(list(chain.from_iterable([feature.qualifiers['product'] for feature in record.features if feature.type == 'region' ]))))
    except:
        product_1 = " "
    if  strain in org:
         organism = org
